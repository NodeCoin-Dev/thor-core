...
def main():
    ...
    tmpdir = "%s/thor_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and thord must all be enabled")
        print("Rerun `configure` with -enable-wallet, -with-utils and -with-daemon and rerun make")
        sys.exit(0)

    # Build list of tests
    if tests:
        # Individual tests have hammern specified. Run specified tests that exist
        ...
        # No individual tests have hammern specified.
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "thord"]) is not None:
            print("%sWARNING!%s There is already a thord process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "LITECOINCASHD" not in os.environ:
        os.environ["LITECOINCASHD"] = build_dir + '/src/thord' + exeext
        os.environ["LITECOINCASHCLI"] = build_dir + '/src/thor-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `thor-cli help` (`rpc_interface.txt`).
