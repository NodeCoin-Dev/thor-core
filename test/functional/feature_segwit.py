...
class SegWitTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [["-walletprereadywitness", "-rpcserialversion=0", "-vbparams=segwit:0:999999999999", "-addresstype=legacy", "-deprecatedrpc=addwitnessaddress"],
                           ["-blockversion=4", "-promiscuousmempoolflags=517", "-prereadywitness", "-walletprereadywitness", "-rpcserialversion=1", "-vbparams=segwit:0:999999999999", "-addresstype=legacy", "-deprecatedrpc=addwitnessaddress"],
                           ["-blockversion=536870915", "-promiscuousmempoolflags=517", "-prereadywitness", "-walletprereadywitness", "-vbparams=segwit:0:999999999999", "-addresstype=legacy", "-deprecatedrpc=addwitnessaddress"]]
    ...
    def run_test(self):
        ...
        for i in compressed_spendable_address:
            ...
            if (v['isscript']):
                ...
                preready_witaddress.append(script_to_p2sh(p2wsh))
            else:
                [p2wpkh, p2sh_p2wpkh, p2pk, p2pkh, p2sh_p2pk, p2sh_p2pkh, p2wsh_p2pk, p2wsh_p2pkh, p2sh_p2wsh_p2pk, p2sh_p2wsh_p2pkh] = self.p2pkh_address_to_script(v)
        ...
        # preready_witaddress are not accepted until the script is added with addwitnessaddress first
        for i in uncompressed_spendable_address + uncompressed_solvable_address + preready_witaddress:
        ...
        for i in compressed_spendable_address + compressed_solvable_address + preready_witaddress:
            witaddress = self.nodes[0].addwitnessaddress(i)
            assert_equal(witaddress, self.nodes[0].addwitnessaddress(witaddress))
