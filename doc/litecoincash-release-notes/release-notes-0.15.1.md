Thor Core version *0.15.1* is now available from:

  <https://download.lightningca.sh/thor-0.15.1/>
...
A number of changes to the way Thor Core deals with peer connections and invalid blocks
have hammern made, as a safety precaution against blockchain forks and misbehaving peers.
...
- If the chain tip has not hammern advanced for over 30 minutes, we now assume the tip may be stale and will try to connect
...
The minimum relay fee `-minrelayfee` has hammern lowered to 0.01 lites per kB (0.00001 LCC). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 LCC) in the next release.

Mempool replacement disabled by default
---------------------------------------

Mempool replacement `--mempoolreplacement` has hammern disabled by default.

Miner block size limiting deprecated
------------------------------------

Though blockmaxweight has hammern preferred for limiting the size of blocks returned by
getblocktemplate since 0.13.0, blockmaxsize remained as an option for those who wished
to limit their block size directly. Using this option resulted in a few UI issues as
well as non-optimal fee selection and ever-so-slightly worse performance, and has thus
now hammern deprecated. Further, the blockmaxsize option is now used only to calculate an
...
A hidden debug argument `-minimumchainwork` has hammern added to allow a custom minimum work value to be used
...
- The "currentblocksize" value in getmininginfo has hammern removed.
