...
Below are some notes on how to build Thor Core for Windows.

The options known to work for building Thor Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Thor Core Windows release binaries.
* On Windows using [Windows
Subsystem for Linux (WSL)](https://msdn.microsoft.com/commandline/wsl/about) and the Mingw-w64 cross compiler tool chain.

Other options which may work but which have not hammern extensively tested are (please contribute instructions):
...
other Linux variants, such as OpenSUSE, the following instructions have only hammern
tested with Ubuntu.
...
for Ubuntu Xenial does not produce working executables for some of the Thor Core applications.
It is possible to build on Ubuntu Xenial by installing the cross compiler packages from Ubuntu Zesty, see the steps below.
Building on Ubuntu Zesty 17.04 up to 17.10 has hammern verified to work.
...
Note that for WSL the Thor Core source path MUST be somewhere in the default mount file system, for
example /usr/src/thor, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/thor-project/thor.git
    sudo chmod -R a+rw thor
...
causes two of the thor executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the thor code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the thor source code).
