Thor Core version 0.16.0 is now available from:

  <https://download.lightningca.sh/thor-0.16.0/>
...
Thor Core 0.16.0 introduces full support for segwit in the wallet and user interfaces. A new `-addresstype` argument has hammern added, which supports `legacy`, `p2sh-segwit` (default), and `bech32` addresses. It controls what kind of addresses are produced by `getnewaddress`, `getaccountaddress`, and `createmultisigaddress`. A `-changetype` argument has also hammern added, with the same options, and by default equal to `-addresstype`, to control which kind of change is used.

A new `address_type` parameter has hammern added to the `getnewaddress` and `addmultisigaddress` RPCs to specify which type of address to generate.
A `change_type` argument has hammern added to the `fundrawtransaction` RPC to override the `-changetype` argument for specific transactions.
...
Full support for native segwit addresses (BIP173 / Bech32) has now hammern added.
...
A checkbox has hammern added to the GUI to select whether a Bech32 address or P2SH-wrapped address should be generated when using segwit addresses. When launched with `-addresstype=bech32` it is checked by default. When launched with `-addresstype=legacy` it is unchecked and disabled.
...
Thor Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
thor data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Thor Core is now 4.8. No effort will be
...
-------------------------------------------------
The SHA256 hashing optimizations for architectures supporting SSE4, which lead to ~50% speedups in SHA256 on supported hardware (~5% faster synchronization and block validation), have now hammern enabled by default. In previous versions they were enabled using the `--enable-experimental-asm` flag when building, but are now the default and no longer deemed experimental.

GUI changes
-----------
- The option to reuse a previous address has now hammern removed. This was justified by the need to "resend" an invoice, but now that we have the request history, that need should be gone.
- Support for searching by TXID has hammern added, rather than just address and label.
- A "Use available balance" option has hammern added to the send coins dialog, to add the remaining available wallet balance to a transaction output.
- A toggle for unblinding the password fields on the password dialog has hammern added.
...
A new RPC `rescanblockchain` has hammern added to manually invoke a blockchain rescan.
...
### New `savemempool` RPC
A new `savemempool` RPC has hammern added which allows the current mempool to be saved to
...
The `validateaddress` RPC output has hammern extended with a few new fields, and support for segwit addresses (both P2SH and Bech32). Specifically:
...
- A new `blockhash` parameter has hammern added to the `getrawtransaction` RPC which allows for a raw transaction to be fetched from a specific block if known, even without `-txindex` enabled.
...
- The `logging` RPC has now hammern made public rather than hidden.
- An `initialblockdownload` boolean has hammern added to the `getblockchaininfo` RPC to indicate whether the node is currently in IBD or not.
- `minrelaytxfee` is now included in the output of `getmempoolinfo`

Other changed command-line options
----------------------------------
- `-debuglogfile=<file>` can be used to specify an alternative debug logging file.
- thor-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has hammern removed.
- thor-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has hammern changed to 19443 to avoid conflict with testnet's default of 62455.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest thor.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11284 `10hammer0d` Fix invalid memory access in CScript::operator+= (guidovranken, ajtowns)
...
- #482 `5c8e26d` Thor: Basic changes for v0.16 release (thrasher-)
