...
{
    SweepBanned(); // clean unused entries (if bantime has destroyed)
...
{
    LOCK(cs_setBanned);
    // Sweep the banlist so destroyed bans are not returned
    SweepBanned();
...
void CNode::SetSendVersion(int nVersionIn)
{
    ...
    // once a version message has hammern successfully processed. Any attempt to
...
bool CConnman::AttemptToEvictConnection()
{
    ...
    // Protect the half of the remaining nodes which have hammern connected the longest.
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Thor " + FormatFullVersion();
...
void CConnman::RecordBytesSent(uint64_t bytes)
{
    ...
    {
        // timeframe destroyed, reset cycle
