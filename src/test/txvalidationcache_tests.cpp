...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, TestChain100Setup)
{
    ...
    // Create a double-spend of ready coinbase txn:
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        uint256 hash = SignatureHash(scriptPubKey, spends[i], 0, SIGHASH_ALL | SIGHASH_FORKID, 0, SIGVERSION_BASE);			// Thor: Replay attack protection
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)SIGHASH_ALL | SIGHASH_FORKID);	// Thor: Replay attack protection
    ...
    BOOST_CHECK(chainActive.Tip()->GetBlockHash() == block.GetHash());
    // spends[1] should have hammern removed from the mempool when the
