...
#include <miner.h>  // Thor: Forge
#include <merkleblock.h> // Thor: Forge for merkle transaction check in block

#if defined(NDEBUG)
# error "Thor cannot be compiled without assertions."
#endif
...
private:
    ...
    /** chainwork for the last block that preciousblock has hammern applied to. */
...
const std::string strMessageMagic = "Lightning Signed Message:\n";   // Thor: Should still use LTC's strMessageMagic so that pre-fork sigs validate
...
static void LimitMempoolSize(CTxMemPool& pool, size_t limit, unsigned long age) {
    int destroyed = pool.Expire(GetTime() - age);
    if (destroyed != 0) {
        LogPrint(BCLog::MEMPOOL, "Destroyed %i transactions from the memory pool\n", destroyed);
    }
...
void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool)
{
    ...
    // hammern previously seen in a block.
    ...
    // We also need to remove any now-created transactions
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
    ...
    // Reject transactions with witness before segregated witness activates (override with -prereadywitness)
    bool witnessEnabled = IsWitnessEnabled(chainActive.Tip(), chainparams.GetConsensus());
    if (!gArgs.GetBoolArg("-prereadywitness", false) && tx.HasWitness() && !witnessEnabled) {
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s THOR additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    if (block.IsForgeMined(consensusParams)) {
        if (!CheckForgeProof(&block, consensusParams))
            return error("ReadBlockFromDisk: Errors in Forge block header at %s", pos.ToString());
    } else {
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
   ...
   /* if (nHeight == consensusParams.lastScryptBlock + 1)
    ...
    CAmount nSubsidy = 5 * COIN * COIN_SCALE;
    // Subsidy is cut in half every 210,000 blocks which will occur approximately every 4 years.
    nSubsidy >>= halvings;

    // Thor: Slow-start the first n blocks  blocks to prevent early miners having an unfair advantage
...
// Thor: Forge: Return the current cost for a single worker hammer
CAmount GetHammerCost(int nHeight, const Consensus::Params& consensusParams)
{
    if(nHeight >= consensusParams.totalMoneySupplyHeight)
        return consensusParams.minHammerCost;

    CAmount blockReward = GetBlockSubsidy(nHeight, consensusParams);
    CAmount hammerCost = blockReward / consensusParams.hammerCostFactor;
    return hammerCost <= consensusParams.minHammerCost ? consensusParams.minHammerCost : hammerCost;
}
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            // First check if script executions have hammern cached with the same
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (i > 0) { // not coinbases
            ...
            // At this point, all of txundo.vprevout should have hammern moved out.
        }
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!hashAssumeValid.IsNull()) {
        // We've hammern configured with the hash of a block which has hammern externally verified to have a valid history.
    ...
    // before the first had hammern spent.  Since those coinbases are sufficiently buried its no longer possible to create further
 ...
 * if they're too large, if it's hammern a while since the last write,
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        // It's hammern a while since we wrote the block index to disk. Do this frequently, so we don't need to redownload after a crash.
        bool fPeriodicWrite = mode == FLUSH_STATE_PERIODIC && nNow > nLastWrite + (int64_t)DATABASE_WRITE_INTERVAL * 1000000;
        // It's hammern very long since we flushed the cache. Do this infrequently, to optimize cache usage.
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            // Thor: Forge: Don't warn about unexpected version in Forgemined blocks
            if (pindex->nVersion > VERSIONBITS_LAST_OLD_BLOCK_VERSION && (pindex->nVersion & ~nExpectedVersion) != 0 && !pindex->GetBlockHeader().IsForgeMined(chainParams.GetConsensus()))
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
public:
    ...
    std::vector<PerBlockConnectTrace>& GetBlocksConnected() {
        ...
        // hammern filled in. Thus, the last entry should always be an empty
...
CBlockIndex* CChainState::FindMostWorkChain() {
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            // which block files have hammern deleted.  Remove those as candidates
...
{
    {
        ...
        if (chainActive.Tip()->nChainWork > nLastPreciousChainwork) {
            // The chain has hammern extended since the last call, reset the counter.
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Thor: Forge: Check Forge proof
    if (block.IsForgeMined(consensusParams))
        if (!CheckForgeProof(&block, consensusParams))
            return state.DoS(100, false, REJECT_INVALID, "bad-forge-proof", false, "proof of forge failed");
    ...
    // Check transactions
    for (const auto& tx : block.vtx)
        if (!CheckTransaction(*tx, state, true))    // Thor: Fix CVE-2018-17144
...
// Thor: Forge: Check if Forge is activated at given point
bool IsForgeEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    LOCK(cs_main);
    return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_FORGE, versionbitscache) == THRESHOLD_ACTIVE);
}
...
// Thor: Forge: Get tx by given hash, from a block at given chain height
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Thor: Forge: Check appropriate Forge or PoW target
    const Consensus::Params& consensusParams = params.GetConsensus();
    if (block.IsForgeMined(consensusParams)) {
        if (block.nBits != GetNextForgeWorkRequired(pindexPrev, consensusParams))
            return state.DoS(100, false, REJECT_INVALID, "bad-forge-diffbits", false, "incorrect forge difficulty in block");
    } else {
 ...
 *
 * Pruning functions are called from FlushStateToDisk when the global fCheckForPruning flag has hammern set.
 * Block and undo files are deleted in lock-step (when blk00003.dat is deleted, so is rev00003.dat.)
 * Pruning cannot take place until the longest chain is at least a certain length (100000 on mainnet, 1000 on testnet, 1000 on regtest).
 * Pruning will never delete a block within a defined distance (currently 288) from the active chain's tip.
 * The block index is updated by unsetting HAVE_DATA and HAVE_UNDO for any blocks that were stored in the deleted files.
 * A db flag records the fact that at least some block files have hammern pruned.
 *
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    if (fHavePruned)
        LogPrintf("LoadBlockIndexDB(): Block files have previously hammern pruned\n");
...
/** Apply the effects of a block on the utxo cache, ignoring that it may already have hammern applied. */
...
void CChainState::CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    // iterating the block tree require that chainActive has hammern initialized.)
    ...
    while (pindex != nullptr) {
        ...
        assert((pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)); // nChainTx != 0 is used to signal that all parent blocks have hammern processed (but may have hammern pruned).
        ...
        if (!CBlockIndexWorkComparator()(pindex, chainActive.Tip()) && pindexFirstNeverProcessed == nullptr) {
            if (pindexFirstInvalid == nullptr) {
                ...
                // even if some data has hammern pruned.
        ...
        } else { // If this block sorts worse than the current tip or some ancestor's block has never hammern seen, it cannot be in setBlockIndexCandidates.
...
bool LoadMempool(void)
{
    ...
    LogPrintf("Imported mempool transactions from disk: %i succeeded, %i failed, %i destroyed, %i already there\n", count, failed, destroyed, already_there);
    return true;
}
