...
const char * const BITCOIN_CONF_FILENAME = "thor.conf";
const char * const BITCOIN_PID_FILENAME = "thord.pid";
...
public:
    CInit()
    {
        ...
        // that the config appears to have hammern loaded and there are no modules/engines available.
        OPENSSL_no_config();
...
/**
 * LogPrintf() has hammern broken a couple of times now
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Thor";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Thor";
#else
    // Unix
    return pathRet / ".thor";
#endif
