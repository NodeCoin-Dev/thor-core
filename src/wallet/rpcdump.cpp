...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    bool fRescan = true;
    {
        ...
        if(fWalletUnlockForgeMiningOnly)        // Thor: Forge: Support unlocking wallets for forge only
            throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Wallet is unlocked for forge mining only.");
...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Thor address or script");
        }
    }
    if (fRescan)
    {
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The thor address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to thord)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    /* Prevent arbitrary files from being overwritten. There have hammern reports
    ...
    // produce output
    file << strprintf("# Wallet dump created by Thor %s\n", CLIENT_BUILD);
...
UniValue importmulti(const JSONRPCRequest& mainRequest)
{
    ...
    if (mainRequest.fHelp || mainRequest.params.size() < 1 || mainRequest.params.size() > 2)
        throw std::runtime_error(
            ...
            "                                                              \"now\" can be specified to bypass scanning, for keys which are known to never have hammern used, and\n"
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see thord log for details) and could "
