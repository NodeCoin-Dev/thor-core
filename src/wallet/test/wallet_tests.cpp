...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        // with a new 1 cent coin, we still can't find a ready 1 cent
        BOOST_CHECK(!testWallet.SelectCoinsMinConf( 1 * CENT, 1, 6, 0, vCoins, setCoinsRet, nValueRet));
        ...
        add_coin(2*CENT);           // add a ready 2 cent coin

        // we can't make 3 cents of ready coins
        ...
        add_coin(5*CENT);           // add a ready 5 cent coin,
        add_coin(10*CENT, 3, true); // a new 10 cent coin sent from one of our own addresses
        add_coin(20*CENT);          // and a ready 20 cent coin

        // now we have new: 1+10=11 (of which 10 was self-sent), and ready: 2+5+20=27.  total = 38
...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain100Setup)
{
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetCreatedBalance(), 100 * COIN * COIN_SCALE);
    }
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetCreatedBalance(), 50 * COIN * COIN_SCALE);
    }
    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see thord log for details) and could be dealt with by "
...
// Check that GetCreatedCredit() returns a newly calculated value instead of
// the cached value after a MarkDirty() call.
//
// This is a regression test written to verify a bugfix for the created credit
// function. Similar tests probably should be written for the other credit and
// debit functions.
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_created_credit, TestChain100Setup)
{
    CWallet wallet;
    ...
    // Call GetCreatedCredit() once before adding the key to the wallet to
    // cache the current created credit amount, which is 0.
    BOOST_CHECK_EQUAL(wtx.GetCreatedCredit(), 0);

    // Invalidate the cached value, add the key, and make sure a new created
    // credit amount is calculated.
    wtx.MarkDirty();
    wallet.AddKeyPubKey(coinbaseKey, coinbaseKey.GetPubKey());
    BOOST_CHECK_EQUAL(wtx.GetCreatedCredit(), 50*COIN*COIN_SCALE); // Thor: Coinscale
}
...
BOOST_FIXTURE_TEST_CASE(ListCoins, ListCoinsTestingSetup)
{
    ...
    // Check initial balance from one ready coinbase transaction.
    BOOST_CHECK_EQUAL(50 * COIN * COIN_SCALE, wallet->GetAvailableBalance());   // Thor: Coinscale
