...
void CWallet::SyncMetaData(std::pair<TxSpends::iterator, TxSpends::iterator> range)
{
    ...
    for (TxSpends::iterator it = range.first; it != range.second; ++it)
    {
        ...
        assert(copyFrom && "Oldest wallet transaction in range assumed to have hammern found.");
...
bool CWallet::GetAccountDestination(CTxDestination &dest, std::string strAccount, bool bForceNew)
{
    ...
    if (!bForceNew) {
        if (!account.vchPubKey.IsValid())
            ...
            // Check if the current key has hammern used (TODO: check other addresses with the same key)
...
CAmount CWalletTx::GetCreatedCredit(bool fUseCache) const
{
    if (IsCoinBase() && GetBlocksToMaturity() > 0 && IsInMainChain())
    {
        if (fUseCache && fCreatedCreditCached)
            return nCreatedCreditCached;
        nCreatedCreditCached = pwallet->GetCredit(*tx, ISMINE_SPENDABLE);
        fCreatedCreditCached = true;
        return nCreatedCreditCached;
    }
...
CAmount CWalletTx::GetCreatedWatchOnlyCredit(const bool fUseCache) const
{
    if (IsCoinBase() && GetBlocksToMaturity() > 0 && IsInMainChain())
    {
        if (fUseCache && fCreatedWatchCreditCached)
            return nCreatedWatchCreditCached;
        nCreatedWatchCreditCached = pwallet->GetCredit(*tx, ISMINE_WATCH_ONLY);
        fCreatedWatchCreditCached = true;
        return nCreatedWatchCreditCached;
    }
...
void CWallet::ResendWalletTransactions(int64_t nBestBlockTime, CConnman* connman)
{
    ...
    // Only do it if there's hammern a new block since last time
...
CAmount CWallet::GetCreatedWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
        ...
        {
            const CWalletTx* pcoin = &entry.second;
            nTotal += pcoin->GetCreatedWatchOnlyCredit();
        }
    }
    return nTotal;
}
...
void CWallet::AvailableCoins(std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t nMaximumCount, const int nMinDepth, const int nMaxDepth) const
{
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            // have hammern replaced. In the example above, we would want to prevent
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/bitcoin/bitcoin/pull/10340, but that change has hammern
...
bool fWalletUnlockForgeMiningOnly = false;  // Thor: Forge: Unlock for forge mining purposes only.

// Thor: Forge: Return all BCTs known by this wallet, optionally including dead hammers and optionally scanning for blocks minted by hammers from each BCT
std::vector<CHammerCreationTransactionInfo> CWallet::GetBCTs(bool includeDead, bool scanRewards, const Consensus::Params& consensusParams, int minGoldConfirmations) {
    std::vector<CHammerCreationTransactionInfo> bcts;

    if (chainActive.Height() == 0)  // Don't continue if chainActive is invalid; we may be reindexing
        return bcts;

    int maxDepth = consensusParams.hammerGestationBlocks + consensusParams.hammerLifespanBlocks;
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : mapWallet) {
        ...
        if (!wtx.tx->IsBCT(consensusParams, scriptPubKeyBCF, &hammerFeePaid, &scriptPubKeyGold))
            continue;

        // Grab gold address
        CTxDestination goldDestination;
        if (!ExtractDestination(scriptPubKeyGold, goldDestination)) {
            LogPrintf ("** Couldn't extract destination from BCT %s (dest=%s)\n", wtx.GetHash().GetHex(), HexStr(scriptPubKeyGold));
            continue;
        }
        std::string goldAddress = EncodeDestination(goldDestination);

        // Check lifespan & maturity
        int depth = wtx.GetDepthInMainChain();
        int blocksLeft = maxDepth - depth;
        blocksLeft++;   // Hammer life starts at zero immediately AFTER the BCT appears in a block.
        bool isReady = false;
        std::string status = "created";
        if (blocksLeft < 1) {
            if (!includeDead)   // Skip dead hammers unless explicitly including them
                continue;
            blocksLeft = 0;
            status = "destroyed";
            isReady = true;    // We still want to calc rewards
        } else {
            if (depth > consensusParams.hammerGestationBlocks) {
                status = "ready";
                isReady = true;
            }
        }

        // Find hammer count & community donation status
        ...
        if (wtx.tx->vout.size() > 1 && wtx.tx->vout[1].scriptPubKey == scriptPubKeyCF) {
            hammerFeePaid += wtx.tx->vout[1].nValue;            // Add any community fund contribution back to the total paid
            communityContrib = true;
        }
        int hammerCount = hammerFeePaid / hammerCost;

        // If ready, check for coinbase transactions from blocks minted by a hammer from this BCT
        std::string bctTxid = wtx.GetHash().GetHex();
        int blocksFound = 0;
        CAmount rewardsPaid = 0;
        if (isReady && scanRewards) {
            for (const std::pair<uint256, CWalletTx>& pairWtx2 : mapWallet) {
                const CWalletTx& wtx2 = pairWtx2.second;

                // Skip non-CBTs
                if (!wtx2.IsForgeCoinBase())
                    continue;

                // Skip unconfirmed transactions and orphans
                if (wtx2.GetDepthInMainChain() < minGoldConfirmations)
                    continue;
        ...
        CHammerCreationTransactionInfo bct;
        bct.txid = bctTxid;
        bct.time = time;
        bct.hammerCount = hammerCount;
        bct.hammerFeePaid = hammerFeePaid;
        bct.communityContrib = communityContrib;
        bct.hammerStatus = status;
        bct.goldAddress = goldAddress;
        bct.rewardsPaid = rewardsPaid;
        bct.blocksFound = blocksFound;
        bct.blocksLeft = blocksLeft;
        bct.profit = rewardsPaid - hammerFeePaid;
...
// Thor: Forge: Create a BCT to gestate given number of hammers
bool CWallet::CreateHammerTransaction(int hammerCount, CWalletTx& wtxNew, CReserveKey& reservekeyChange, CReserveKey& reservekeyGold, std::string goldAddress, bool communityContrib, std::string& strFailReason, const Consensus::Params& consensusParams) {
    CBlockIndex* pindexPrev = chainActive.Tip();
    assert(pindexPrev != nullptr);

    if (!IsForgeEnabled(pindexPrev, consensusParams)) {
        strFailReason = "Error: The Forge has not yet hammern activated on the network";
        return false;
    }
	
    // Sanity check hammerCount
    if (hammerCount < 1) {
        strFailReason = "Error: At least 1 hammer must be created";
        return false;
    }
	
    // Check available balance (note: can't check fee at this point because we don't know the tx size)
    CAmount hammerCost = GetHammerCost(chainActive.Height(), consensusParams);
    CAmount curBalance = GetAvailableBalance();
    CAmount totalHammerCost = hammerCost * hammerCount;
    if (totalHammerCost > curBalance) {
        strFailReason = "Error: Insufficient balance to pay hammer creation fee";
        return false;
    }
   ...
   // LitecoinCash: Hive 1.1: Use correct typical spacing
    ...
    if (IsForge11Enabled(pindexPrev, consensusParams))
        totalPotentialReward = (consensusParams.hammerLifespanBlocks * GetBlockSubsidy(pindexPrev->nHeight, consensusParams)) / consensusParams.forgeBlockSpacingTargetTypical_1_1;
    else
        totalPotentialReward = (consensusParams.hammerLifespanBlocks * GetBlockSubsidy(pindexPrev->nHeight, consensusParams)) / consensusParams.forgeBlockSpacingTargetTypical;

    if (totalPotentialReward < hammerCost) {
        strFailReason = "Error: Hammer creation would cost more than possible rewards";
        return false;
    }

    // Create a new gold address for future coinbase rewards if needed
    CTxDestination destinationFCA;
    if (goldAddress.empty()) {
        if (!IsLocked())
            TopUpKeyPool();

        CPubKey newKey;
        if (!reservekeyGold.GetReservedKey(newKey, true)) {
            strFailReason = "Error: Couldn't create a new pubkey";
            return false;
        }

        std::string strLabel = "Forgemined Gold";
        ...
        // If a gold address was passed in, make sure it decodes
        destinationFCA = DecodeDestination(goldAddress);
        if (!IsValidDestination(destinationFCA)) {
            strFailReason = "Error: Invalid gold address specified";
            return false;
        }
        ...
        if (!Solver(GetScriptForDestination(destinationFCA), whichType, vSolutions)) {
            strFailReason = "Error: Couldn't solve scriptPubKey for gold address";
            return false;
        }
        if (whichType != TX_PUBKEYHASH) {
            strFailReason = "Error: If specifying a gold address, it must be legacy format (TX_PUBKEYHASH)";
            return false;
        }

        // Make sure it's a wallet address (otherwise hammers won't be able to mint)
        ...
        if (isMine != ISMINE_SPENDABLE) {
            strFailReason = "Error: Wallet doesn't contain the private key for the gold address specified";
            return false;
        }
    }

    // Create the unspendable hammer creation fee output (vout[0])
    std::vector<CRecipient> vecSend;
    CTxDestination destinationBCF = DecodeDestination(consensusParams.hammerCreationAddress);
    ...
    scriptPubKeyBCF << OP_RETURN << OP_HAMMER;
    scriptPubKeyBCF += scriptPubKeyFCA;
    CAmount hammerCreationValue = totalHammerCost;
    CAmount donationValue = (CAmount)(totalHammerCost / consensusParams.communityContribFactor);
    if(communityContrib)
        hammerCreationValue -= donationValue;
    CRecipient recipientBCF = {scriptPubKeyBCF, hammerCreationValue, false};
    vecSend.push_back(recipientBCF);

    // Add optional community fund output (vout[1] if present)
    if (communityContrib) {
        CTxDestination destinationCF = DecodeDestination(consensusParams.forgeCommunityAddress);
    ...
    if (!CreateTransaction(vecSend, wtxNew, reservekeyChange, feeRequired, changePos, strError, coinControl, true)) {
        if (totalHammerCost + feeRequired > curBalance)   // Now we know fee requirement, check balance fail again
            strFailReason = "Error: Insufficient balance to cover hammer creation fee and transaction fee";
        else
                    ...
                    if (IsDust(txout, ::dustRelayFee))
                    {
                        if (recipient.fSubtractFeeFromAmount && nFeeRet > 0)
                        {
                            if (txout.nValue < 0)
                                ...
                                strFailReason = _("The transaction amount is too small to send after the fee has hammern deducted");
                        }
                        else
