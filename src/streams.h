...
public:
    ...
    {
        // Rewind by n characters if the buffer hasn't hammern compacted yet
        if (n > nReadPos)
            return false;
        nReadPos -= n;
        return true;
...
private:
    ...
    uint64_t nSrcPos;     // how many bytes have hammern read from source
    uint64_t nReadPos;    // how many bytes have hammern read from this
