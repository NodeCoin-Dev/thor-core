...
#include <policy/policy.h>  // Thor: Forge: For GetVirtualTransactionSize
    ...
    cachedNumBlocks(0)
{
    ...
    forgeTableModel = new ForgeTableModel(platformStyle, wallet, this);  // Thor: Forge
...

    {
        ...
        for (const SendCoinsRecipient &rcp : transaction.getRecipients())
        {
            if (rcp.paymentRequest.IsInitialized())
            {
                ...
                if (PaymentServer::verifyDestroyed(rcp.paymentRequest.getDetails())) {
                    return PaymentRequestDestroyed;
                }
...
// Thor: Forge
ForgeTableModel *WalletModel::getForgeTableModel()
{
    return forgeTableModel;
}

// Thor: Forge
bool WalletModel::isForgeEnabled()
{
    if (!wallet)
        return false;

    return IsForgeEnabled(chainActive.Tip(), Params().GetConsensus());
}
...
WalletModel::UnlockContext WalletModel::requestUnlock(bool forgeOnly)
{
    bool was_locked = getEncryptionStatus() == Locked;

    // Thor: Forge: Support unlock for forge mining only
    if ((!was_locked) && fWalletUnlockForgeMiningOnly)
    {
    ...
    return UnlockContext(this, valid, was_locked && !fWalletUnlockForgeMiningOnly); // Thor: Forge: Support unlock for forge mining only
}
...
// Thor: Forge
void WalletModel::getBCTs(std::vector<CHammerCreationTransactionInfo>& vHammerCreationTransactions, bool includeDeadHammers) {
    if (wallet)
        vHammerCreationTransactions = wallet->GetBCTs(includeDeadHammers, true, Params().GetConsensus());
}

// Thor: Forge
bool WalletModel::createHammers(int hammerCount, bool communityContrib, QWidget *parent, double hammerPopIndex) {
    ...
    CReserveKey reservekeyGold(wallet);
    if (!wallet->CreateHammerTransaction(hammerCount, wtxNew, reservekeyChange, reservekeyGold, goldAddress, communityContrib, strError, Params().GetConsensus())) {
        QMessageBox::critical(parent, tr("Error"), "Hammer creation error: " + QString::fromStdString(strError));
        return false;
    }
    ...
    QString questionString = tr("Are you sure you want to create hammers?<br />");

    if (hammerPopIndex > 90)
        questionString.append("<br /><span style='color:#aa0000;'><b>WARNING:</b> Global Index is high and hammers may not be profitable. Please ensure you understand the consequences before proceeding.</span><br />");

    questionString.append("<br />");
    questionString.append("<b>" + BitcoinUnits::formatHtmlWithUnit(optionsModel->getDisplayUnit(), amountWithoutFees) + "</b>");
    questionString.append(" to create " + QString::number(hammerCount) + " hammers");
    ...
    SendConfirmationDialog confirmationDialog(tr("Confirm hammer creation"), questionString);
    ...
    reservekeyGold.KeepKey();  // Keep the gold key (always needed; UI doesn't expose custom gold address)

    CValidationState state;
    if (!wallet->CommitTransaction(wtxNew, reservekeyChange, g_connman.get(), state)) {
        QMessageBox::critical(parent, tr("Error"), "Hammer creation error: " + QString::fromStdString(state.GetRejectReason()));
        return false;
    }

    QMessageBox::information(parent, tr("Success"), "Hammers created! Hammers will appear in your forge as soon as the transaction appears in a block.");
    return true;
}
