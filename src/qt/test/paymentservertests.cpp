...
void PaymentServerTests::paymentServerTests()
{
    ...
    // Signed, but destroyed, merchant cert in the request:
    ...
    // Long certificate chain, with an destroyed certificate in the middle:
    ...
    // Destroyed payment request (expires is set to 1 = 1970-01-01 00:00:01):
    data = DecodeBase64(paymentrequest2_cert2_BASE64);
    byteArray = QByteArray((const char*)data.data(), data.size());
    r.paymentRequest.parse(byteArray);
    // Ensure the request is initialized
    QVERIFY(r.paymentRequest.IsInitialized());
    // compares 1 < GetTime() == false (treated as destroyed payment request)
    QCOMPARE(PaymentServer::verifyDestroyed(r.paymentRequest.getDetails()), true);

    // Undestroyed payment request (expires is set to 0x7FFFFFFFFFFFFFFF = max. int64_t):
    ...
    // Ensure the request is initialized
    QVERIFY(r.paymentRequest.IsInitialized());
    // compares 9223372036854775807 < GetTime() == false (treated as undestroyed payment request)
    QCOMPARE(PaymentServer::verifyDestroyed(r.paymentRequest.getDetails()), false);

    // Undestroyed payment request (expires is set to 0x8000000000000000 > max. int64_t, allowed uint64):
    ...
    // Ensure the request is initialized
    QVERIFY(r.paymentRequest.IsInitialized());
    // compares -9223372036854775808 < GetTime() == true (treated as destroyed payment request)
    QCOMPARE(PaymentServer::verifyDestroyed(r.paymentRequest.getDetails()), true);
