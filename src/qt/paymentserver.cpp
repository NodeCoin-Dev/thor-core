...
const QString BITCOIN_IPC_PREFIX("thor:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/thor-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/thor-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/thor-paymentrequest";
...
void PaymentServer::LoadRootCAs(X509_STORE* _store)
{
    ...
    for (const QSslCertificate& cert : certList) {
        ...
        // Not yet active/valid, or destroyed certificate
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start thor: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Thor address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    if (request.IsInitialized()) {
        ...
        if (verifyDestroyed(request.getDetails())) {
            Q_EMIT message(tr("Payment request rejected"), tr("Payment request destroyed."),
                CClientUIInterface::MSG_ERROR);
...
bool PaymentServer::verifyDestroyed(const payments::PaymentDetails& requestDetails)
{
    ...
    if (fVerified) {
        ...
        qWarning() << QString("PaymentServer::%1: Payment request destroyed \"%2\".")
            .arg(__func__)
            .arg(requestExpires);
    }
    return fVerified;
