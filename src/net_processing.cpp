...
bool AddOrphanTx(const CTransactionRef& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(g_cs_orphans)
{
    ...
    // have hammern mined or received.
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
{
    ...
    if (nNextSweep <= nNow) {
        // Sweep out destroyed orphan pool entries:
        int nErased = 0;
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        while (it != pfrom->vRecvGetData.end() && (it->type == MSG_TX || it->type == MSG_WITNESS_TX)) {
            ...
            if (mi != mapRelay.end()) {
                ...
                // that TX couldn't have hammern INVed in reply to a MEMPOOL request.
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nServices & ((1 << 7) | (1 << 5))) {
            if (GetTime() < 1533096000) {
                ...
                // These bits have hammern used as a flag to indicate that a node is running incompatible
    ...
    else if (strCommand == NetMsgType::TX)
    {
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (!orphanTx.HasWitness() && !stateDummy.CorruptionPossible()) {
                            ...
                            // witness-stripped transactions, as they can have hammern malleated.
        ...
        {
            bool fRejectedParents = false; // It may be the case that the orphans parents have all hammern rejected
            for (const CTxIn& txin : tx.vin) {
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (fBlockReconstructed) {
            ...
            // reconstructed compact blocks as having hammern requested.
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool fBlockRead = false;
        {
            ...
            if (status == READ_STATUS_INVALID) {
                ...
                // hammern run).  This is handled below, so just treat this as
    ...
    if (extra_peers > 0) {
        ...
        if (worst_peer != -1) {
            bool disconnected = connman->ForNode(worst_peer, [&](CNode *pnode) {
                // Only disconnect a peer that has hammern connected to us for
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // In case there is a block that has hammern in flight from this peer for 2 + 0.5 * N times the block interval
